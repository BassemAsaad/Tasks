...
public class ValidationExceptionHandler {
    ...
    /**
     * Handles the ValidationException.
     *
     * @param e is the ValidationException.
     * @return data responded to the client.
     */
    @ExceptionHandler(ValidationException.class)
    public GenericResponse<List<ErrorResponse>> handleNotFoundException(final ValidationException e) {

        final List<ErrorResponse> errors = e.getErrorMessages()
                .entrySet()
                .stream()
                .map(entry -> new ErrorResponse(entry.getKey(), entry.getValue()))
                .toList();

        return new GenericResponse<>(2001, e.getTimestamp(), errors);
    }
}
